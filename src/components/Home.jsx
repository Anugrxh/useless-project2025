...
// V6: Kerala Style Road Animation (This component is unchanged)
const RoadAnimation = () => {
  /* ... Your existing RoadAnimation code ... */
...
      setError(null);
      handleUpload(file); // Trigger the upload immediately after validation
    } else {
...
    formData.append('file', file); // The key 'file' must match what your server expects
...
      if (!response.ok) {
        // If the server returns an error, try to read it as text
        const errText = await response.text();
        throw new Error(`Server error: ${response.status} - ${errText}`);
      }

      // Assuming the server responds with the processed image file directly
...
      setStatus('idle'); // Reset the state to allow the user to try again
    }
  };
...
          {(status === 'idle' && !error) && ( // Simplified idle check
            <motion.div
              key="idle"
...
          )}
          ...
          {error && (
            <motion.div key="error" className="text-center p-4 bg-red-100 border border-red-400 text-red-700 rounded-lg">
                <p>{error}</p>
                <button onClick={resetState} className="mt-4 text-sm font-semibold underline">Try again</button>
            </motion.div>
          )}
...
        <AnimatePresence>
          {/* This image viewer modal is unchanged */}
          {showViewer && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
